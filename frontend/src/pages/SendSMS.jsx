...
import { sendBulkSMS } from '../services/termiiService'
//import { sendBulkSMSTwilio } from '../services/twilioService'
import { sendBulkEmails } from '../services/emailService'
import { scheduleMessagesForManifest } from '../services/smsScheduler'
...
async function handleSendSMS() {
  if (sendOption === 'scheduled') {
    ...
    try {
      ...
      // Optionally redirect to scheduled messages page
      setTimeout(() => {
        window.location.href = '/scheduled-messages'
      }, 2000)
    ...
    try {
      // CREATE manifestData OBJECT HERE (BEFORE using it)
      const manifestData = {
        company: company.company_name,
        departure: route.departure_location,
        destination: route.destination,
        ...
        {sendOption === 'scheduled' && (
          <div className="mt-3 bg-blue-50 border-l-4 border-blue-600 p-3 rounded">
            <p className="text-sm text-blue-900">
              <strong>Smart Scheduling:</strong> Messages will be automatically sent:
            </p>
            <ul className="text-sm text-blue-800 mt-2 space-y-1">
              <li>• Passenger: 30 mins after departure</li>
              <li>• Next of Kin: 30 mins after departure</li>
              <li>• Final message: 30 mins before arrival</li>
            </ul>
          </div>
        )}
      </div>
